...
class DemandForecastModel:
    ...
    def tune_hyperparameters(
        ...
        """
        Tune LightGBM + XGBoost hyperparameters with a compact Optuna search.
    ...
    def _build_optuna_config(self, trial: Any) -> ForecastModelConfig:
        ...
        xgboost_params = base_payload["xgboost_params"].copy()

        lgb_rounds = trial.suggest_int("lightgbm_n_estimators", 60, 260, step=40)
        xgb_rounds = trial.suggest_int("xgboost_n_estimators", 60, 260, step=40)
        ...
        xgboost_params.update(
            {
                ...
                "learning_rate": trial.suggest_float("xgboost_learning_rate", 0.02, 0.12, log=True),
                "max_depth": trial.suggest_int("xgboost_max_depth", 3, 8),
                "subsample": trial.suggest_float("xgboost_subsample", 0.7, 1.0),
                "colsample_bytree": trial.suggest_float("xgboost_colsample_bytree", 0.7, 1.0),
                "reg_lambda": trial.suggest_float("xgboost_reg_lambda", 0.3, 5.0, log=True),
                "min_child_weight": trial.suggest_categorical(
                    "xgboost_min_child_weight", [1, 3, 5]
                ),
            }
        )
        ...
        base_payload["xgboost_params"] = xgboost_params
        base_payload["ensemble_weights"] = {
            "lightgbm": lightgbm_weight,
            "xgboost": 1.0 - lightgbm_weight,
        }
